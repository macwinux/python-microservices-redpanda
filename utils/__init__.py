...
def log_exception(message: str, sys_exec_info) -> None:
    exc_type, exc_obj, exc_tb = sys_exec_info
...
def get_system_config(
    ...
    try:
        ...
        for k, v in sys_config["status_id"].items():
            sys_config["status_id"][k] = int(v)
...
def validate_cli_args(script: str) -> tuple:
    ...
    if len(sys.argv):
        logging.error(
            (
                f"Missing configuration files. Usage: {script}.py {{KAKFA_CONFIG_FILE}} {{SYS_CONFIG_FILE}}\n"
                "Where:\n"
                " - KAFKA_CONFIG_FILE: file under the folder 'config_kafka/'\n"
                f" - SYS_CONFIG_FILE: file under the folder 'config_suys/' (default file is '{DEFAULT_SYS_CONFIG}')\n"
            )
        )
        sys.exit(0)
    ...
    if not os.path.isfile(kafka_config_file):
        logging.error(f"Kakfa configuration file not found: {kafka_config_file}\n")
        sys.exit(0)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.was_signal_set:
            if self.consumer is not None:
                try:
                    # Close down consumer to commit final offset
